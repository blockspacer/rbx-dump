...
renderSteppedConnection = game:GetService("RunService").RenderStepped:connect(function()
	...
	if infoFrame then
		local screenSize = currScreenGui.AbsoluteSize
		...
		currScreenGui.BlackFrame.ThumbnailContainer.Size = thumbnailSize
		currScreenGui.BlackFrame.ThumbnailContainer.Position = thumbnailPosition
		currScreenGui.BlackFrame.ThumbnailContainer.LoadingInfoContainer.Visible = (screenSize.Y > 500)
		
		local instanceCount = guiService:GetInstanceCount()
		local voxelCount = guiService:GetVoxelCount()
		local brickCount = guiService:GetBrickCount()
		local connectorCount = guiService:GetConnectorCount()
		
		currScreenGui.BlackFrame.ThumbnailContainer.LoadingInfoContainer.InstancesValue.Text = tostring(instanceCount)
		currScreenGui.BlackFrame.ThumbnailContainer.LoadingInfoContainer.BricksValue.Text = tostring(brickCount)
		currScreenGui.BlackFrame.ThumbnailContainer.LoadingInfoContainer.ConnectorsValue.Text = tostring(connectorCount)
		
		if voxelCount <= 0 then
			currScreenGui.BlackFrame.ThumbnailContainer.LoadingInfoContainer.VoxelsValue.Text = "0"
		else
			currScreenGui.BlackFrame.ThumbnailContainer.LoadingInfoContainer.VoxelsValue.Text = tostring(voxelCount) .." million"
		end
